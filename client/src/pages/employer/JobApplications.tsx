...
export default function JobApplications() {
    ...
    try {
      ...
      if (jobId) {
        ...
        const transformedApplications = (data.applications || []).map((app: {
          ...
          id: app.id,
          applicantName: app.jobSeeker ? `${app.jobSeeker.firstName} ${app.jobSeeker.lastName}` : 'Unknown',
        ...
        const data = response.data as { applications: Application[]; pagination?: { current: number; total: number; totalItems: number; hasNext: boolean; hasPrev: boolean } };
      ...
      setApplications([]);
      setJob(jobId ? { id: jobId, title: 'Unknown Job', applicationsCount: 0 } : null);
  ...
  const getStatusBadgeColor = (status: string) => {
    switch (status) {
      ...
      case 'REVIEWING': return 'text-blue-600 dark:text-blue-400';
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {['ALL', 'PENDING', 'REVIEWING', 'ACCEPTED', 'REJECTED'].map((status) => (
            <button
              key={status}
              onClick={() => setFilter(status as 'ALL' | 'PENDING' | 'REVIEWING' | 'ACCEPTED' | 'REJECTED')}
          ...
          {filteredApplications.length === 0 ? (
            ...
            filteredApplications.map((application, index) => (
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    {!jobId && application.jobTitle && (
                      <div className="bg-muted/50 rounded-lg p-3 mb-4">
                        <h4 className="font-medium text-foreground mb-1">Applied for:</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-muted-foreground text-sm mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{application.user?.email || application.applicantEmail}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <MdLocationOn className="w-4 h-4" />
                        <span>{application.user?.profile?.location || application.location}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <MdWork className="w-4 h-4" />
                        <span>{application.user?.profile?.experience || application.experience}</span>
                      </div>
                    </div>
                ...
                {/* Skills */}
                {(application.user?.profile?.skills || application.skills)?.length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(application.user?.profile?.skills || application.skills).map((skill, skillIndex) => (
                        <span
                          key={skillIndex}
        ...
        {applications.length > 0 && (
          ...
          >
            ...
            <div className="bg-card p-6 rounded-xl border border-border">
              <div className={`text-3xl font-bold ${getStatusBadgeColor('PENDING')}`}>
                {applications.filter(a => a.status === 'PENDING').length}
              </div>
              <div className="text-muted-foreground text-sm">Pending</div>
            </div>
            <div className="bg-card p-6 rounded-xl border border-border">
              <div className={`text-3xl font-bold ${getStatusBadgeColor('REVIEWING')}`}>
                {applications.filter(a => a.status === 'REVIEWING').length}
              </div>
              <div className="text-muted-foreground text-sm">Reviewing</div>
            </div>
            <div className="bg-card p-6 rounded-xl border border-border">
              <div className={`text-3xl font-bold ${getStatusBadgeColor('ACCEPTED')}`}>
                {applications.filter(a => a.status === 'ACCEPTED').length}
              </div>
              <div className="text-muted-foreground text-sm">Accepted</div>
            </div>
            <div className="bg-card p-6 rounded-xl border border-border">
              <div className={`text-3xl font-bold ${getStatusBadgeColor('REJECTED')}`}>
