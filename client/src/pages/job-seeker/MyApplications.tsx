        ...
        {filteredApplications.length === 0 ? (
          ...
          >
            ...
            {filter === 'ALL' && (
              <Link
                to="/jobs"
                className="bg-primary text-white px-6 py-2 rounded-md hover:bg-primary-600 transition-colors inline-block"
              >
                Browse Jobs
              </Link>
            )}
            ...
            >
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span className={application.status === 'PENDING' ? 'text-primary font-medium' : ''}>
                    Applied
                  </span>
                  <span className={application.status === 'REVIEWING' ? 'text-primary font-medium' : ''}>
                    Under Review
                  </span>
                  <span className={['ACCEPTED', 'REJECTED'].includes(application.status) ? 'text-primary font-medium' : ''}>
                    Decision
                  </span>
                </div>
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full transition-all duration-300 ${
                      application.status === 'PENDING' ? 'w-1/3 bg-yellow-500' :
                      application.status === 'REVIEWING' ? 'w-2/3 bg-blue-500' :
                      application.status === 'ACCEPTED' ? 'w-full bg-green-500' :
                      application.status === 'REJECTED' ? 'w-full bg-red-500' :
                      'w-1/3 bg-gray-400'
                    }`}
                  />
                </div>
      ...
      {applications.length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
            <div>
              <strong>⏳ Pending:</strong> Your application has been submitted and is waiting to be reviewed.
            </div>
            <div>
              <strong>👀 Under Review:</strong> The employer is currently reviewing your application.
            </div>
            <div>
              <strong>✅ Accepted:</strong> Congratulations! The employer is interested in moving forward.
            </div>
            <div>
              <strong>❌ Rejected:</strong> Unfortunately, you weren't selected for this position.
            </div>
          </div>
        </motion.div>
      )}
