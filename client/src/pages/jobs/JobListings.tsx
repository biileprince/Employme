...
import { HiFilter, HiLocationMarker, HiClock, HiBriefcase, HiCurrencyDollar, HiStar, HiX } from 'react-icons/hi';
...
const JobListings = () => {
  ...
  const [locationResults, setLocationResults] = useState<{place_id: number; display_name: string}[]>([]);
    ...
    try {
      const response = await fetch(
        `${LOCATION_API_CONFIG.baseUrl}?format=json&q=${encodeURIComponent(
          query
        )}&limit=${LOCATION_API_CONFIG.limit}&countrycodes=${LOCATION_API_CONFIG.countryCodes}`
      );
  ...
  const selectLocation = (location: {place_id: number; display_name: string}) => {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      ...
      if (filter.jobType) params.jobType = filter.jobType.toUpperCase().replace('-', '_');
  ...
  // Mock company logos for demonstration
  const getCompanyLogo = (companyName: string) => {
    const logos: Record<string, string> = {
      'TechCorp Ghana': 'https://images.unsplash.com/photo-1549923746-c502d488b3ea?w=64&h=64&fit=crop&crop=center',
      'DigitalWave': 'https://images.unsplash.com/photo-1572021335469-31706a17aaef?w=64&h=64&fit=crop&crop=center',
      'GhanaFintech': 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=64&h=64&fit=crop&crop=center',
      'BuildTech Solutions': 'https://images.unsplash.com/photo-1551434678-e076c223a692?w=64&h=64&fit=crop&crop=center',
      'DataInsight Ghana': 'https://images.unsplash.com/photo-1551836022-deb4988cc6c0?w=64&h=64&fit=crop&crop=center',
    };
    return logos[companyName] || 'https://images.unsplash.com/photo-1549923746-c502d488b3ea?w=64&h=64&fit=crop&crop=center';
  };
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    const matchesJobType = !filter.jobType || job.jobType.toLowerCase().replace('_', '-') === filter.jobType.toLowerCase();
    const matchesLocation = !filter.location || job.location.toLowerCase().includes(filter.location.toLowerCase());
    
    const matchesCategories = filter.categories.length === 0 || 
      filter.categories.some(category => job.category.toLowerCase() === category.toLowerCase());
    ...
    return matchesSearch && matchesJobType && matchesLocation && matchesCategories && matchesSalary;
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-br from-primary via-primary to-primary-600 text-white py-16 relative overflow-hidden"
           style={{
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-white/95 max-w-2xl mx-auto drop-shadow-md">
              {loading ? 'Loading jobs...' : `${filteredJobs.length} jobs available across Ghana`}
            </p>
          </div>
                      ...
                      {locationResults.map((location) => (
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <span className="truncate">{location.display_name.split(',')[0].trim()}</span>
                          </div>
                        </button>
                      ))}
                    </div>
          ...
          <div className={`lg:w-80 space-y-6 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="bg-card dark:bg-gray-800 rounded-xl border border-border dark:border-gray-700 p-6 sticky top-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground dark:text-white">Filter Jobs</h3>
                <button
              ...
              {filter.categories.length > 0 && (
                <div className="mb-4">
                  <h4 className="font-medium mb-2 text-foreground dark:text-white">Selected Categories:</h4>
              ...
              <div className="mb-6">
                <h4 className="font-medium mb-3 text-foreground dark:text-white">Categories</h4>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {jobCategories.map(category => (
                    <label key={category} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded-lg">
                      <div className="flex items-center">
                        <input 
                          type="checkbox" 
              ...
              {/* Job Type */}
              <div className="mb-6">
                <h4 className="font-medium mb-3 text-foreground dark:text-white">Job Type</h4>
                <div className="space-y-2">
                  {['full-time', 'part-time', 'contract', 'internship'].map(type => (
                    <label key={type} className="flex items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded-lg">
                      <input 
                        type="radio" 
                        name="jobType"
                        value={type}
              ...
              {/* Experience Level */}
              <div className="mb-6">
                <h4 className="font-medium mb-3 text-foreground dark:text-white">Experience Level</h4>
                <div className="space-y-2">
                  {['Entry Level', 'Mid Level', 'Senior Level', 'Executive'].map(level => {
                    const isSelected = filter.experience === level;
                    return (
                      <label key={level} className="flex items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded-lg">
                        <input 
                          type="radio" 
                          name="experience"
                          value={level}
                          ...
                          className="mr-3 text-primary focus:ring-primary"
                        />
                        <span className={`text-sm transition-colors ${isSelected ? 'text-primary font-medium' : 'text-muted-foreground hover:text-foreground'} dark:text-gray-300 dark:hover:text-white`}>
                          {level}
                        </span>
                      </label>
                    );
              ...
              {/* Salary Range */}
              <div>
                <h4 className="font-medium mb-3 text-foreground dark:text-white">Salary Range</h4>
                <div className="space-y-2">
                  {['Under GHS 2,000', 'GHS 2,000 - 5,000', 'GHS 5,000 - 10,000', 'Above GHS 10,000'].map(range => (
                    <label key={range} className="flex items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded-lg">
                      <input 
                        type="radio" 
                        name="salaryRange"
                        value={range}
                        ...
                        className="mr-3 text-primary focus:ring-primary"
                      />
                      <span className="text-sm text-muted-foreground hover:text-foreground dark:text-gray-300 dark:hover:text-white">{range}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-border dark:border-gray-600">
                <Button 
                  variant="outline" 
                  fullWidth 
                  onClick={() => setFilter({ jobType: '', location: '', salaryRange: '', experience: '', categories: [] })}
                >
                  Clear All Filters
                </Button>
              </div>
              ...
              <div className="space-y-4">
                {filteredJobs.length === 0 ? (
                  ...
                  filteredJobs.map(job => (
                    <div key={job.id} className="bg-card border border-border rounded-xl p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                      <div className="flex flex-col sm:flex-row gap-4">
                        
                        {/* Company Logo */}
                        <div className="flex-shrink-0 relative">
                          {/* Enhanced decorative background elements */}
                          <div className="absolute -top-2 -right-2 w-6 h-6 bg-secondary-500/20 rounded-full z-50"></div>
                          <div className="absolute -bottom-1 -left-1 w-4 h-4 bg-primary-500/15 rotate-45 z-50"></div>
                          
                          <img 
                            src={getCompanyLogo(job.employer.companyName)} 
                            alt={`${job.employer.companyName} logo`}
                            className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl object-cover border-2 border-border hover:border-primary/50 transition-all duration-300 relative z-10 shadow-md hover:shadow-lg"
                          />
                          
                          {/* Enhanced pattern overlay */}
                          <div className="absolute inset-1 rounded-lg overflow-hidden z-50">
                            <div className="absolute top-1 right-1 w-2 h-2 border border-secondary-400/40 rounded-full bg-white/10 z-50"></div>
                            <div className="absolute bottom-1 left-1 w-3 h-3 bg-primary-400/30 rotate-45 rounded-sm z-50"></div>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-lg text-foreground font-medium">{job.employer.companyName}</p>
                            </div>
