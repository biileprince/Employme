...
export const getJobs = catchAsync(async (req: Request, res: Response): Promise<void> => {
  const { 
    page = 1, 
    limit = 10, 
    category,
...
// Get job by ID
export const getJobById = catchAsync(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
...
export const getMyJobs = catchAsync(async (req: Request, res: Response): Promise<void> => {
  if (!req.user) {
...
export const createJob = [
  ...
  body('category').isIn([
    ...
    'DESIGN', 'ENGINEERING', 'OPERATIONS', 'HUMAN_RESOURCES', 'LEGAL', 
    'CUSTOMER_SERVICE', 'MANUFACTURING', 'CONSULTING', 'MEDIA', 'GOVERNMENT', 
    'NON_PROFIT', 'AGRICULTURE', 'CONSTRUCTION', 'HOSPITALITY', 'TRANSPORTATION', 
    'RETAIL', 'REAL_ESTATE', 'TELECOMMUNICATIONS', 'OTHER'
  ]).withMessage('Invalid job category'),
  body('experienceLevel').optional().isIn(['ENTRY_LEVEL', 'MID_LEVEL', 'SENIOR_LEVEL', 'EXECUTIVE'])
    .withMessage('Invalid experience level'),
  body('type').isIn(['FULL_TIME', 'PART_TIME', 'CONTRACT', 'INTERNSHIP', 'FREELANCE'])
  ...
  body('contactPhone').optional().matches(/^[0-9]{7,15}$/).withMessage('Contact phone must be 7-15 digits'),
  ...
  catchAsync(async (req: Request, res: Response): Promise<void> => {
    ...
    if (!employer) {
      throw new AppError('Employer profile not found. Please complete your employer profile first.', 404);
    }

    const employerId = employer.id;

    // Format contact phone if provided
    const formattedContactPhone = contactPhone && contactCountryCode 
...
export const updateJob = [
  ...
  body('location').optional().notEmpty().withMessage('Job location cannot be empty'),
  body('experience').optional().isIn(['ENTRY_LEVEL', 'MID_LEVEL', 'SENIOR_LEVEL'])
    .withMessage('Invalid experience level'),
  body('jobType').optional().isIn(['FULL_TIME', 'PART_TIME', 'CONTRACT', 'INTERNSHIP'])
    .withMessage('Invalid job type'),
  body('salaryMin').optional().isNumeric().withMessage('Salary min must be a number'),
  body('salaryMax').optional().isNumeric().withMessage('Salary max must be a number'),
...
export const deleteJob = catchAsync(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
