...
export const login = catchAsync(async (req: Request, res: Response): Promise<void> => {
  ...
  const hasProfile = !!(user.role === 'JOB_SEEKER' && user.jobSeeker) || 
...
export const requestPasswordReset = catchAsync(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    await sendPasswordResetEmail(user.email, user.firstName || 'User', resetCode);
  } catch (error) {
...
export const socialAuthSuccess = catchAsync(async (req: Request, res: Response): Promise<void> => {
  if (!req.user) {
...
export const socialAuthFailure = catchAsync(async (req: Request, res: Response): Promise<void> => {
  const frontendUrl = process.env.CLIENT_URL || 'http://localhost:5173';
  res.redirect(`${frontendUrl}/auth?error=social_auth_failed`);
});

// Link Social Account (for existing users)
export const linkSocialAccount = catchAsync(async (req: Request, res: Response): Promise<void> => {
  if (!req.user) {
    throw new AppError('User not authenticated', 401);
  }
...
export const unlinkSocialAccount = catchAsync(async (req: Request, res: Response): Promise<void> => {
  if (!req.user) {
    throw new AppError('User not authenticated', 401);
  }
  ...
  if (!user.password && user.socialAccounts && user.socialAccounts.length === 1 && user.socialAccounts[0].provider === provider) {
    throw new AppError('Cannot unlink last social account without setting a password first', 400);
  }
