...
export default function EditJobPage() {
  ...
  const handleJobImageUpload = async (files: File[]) => {
    try {
      ...
      formDataToSend.append("type", "JOB");

      const response = await apiClient.post<{
        attachments: Array<{ id: string; url: string; filename: string }>;
      }>("/attachments/upload", formDataToSend);

      if (response.success && response.data) {
        setFormData((prev) => ({
          ...prev,
          jobImages: response.data!.attachments.map((att) => att.url),
        }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const jobData = {
        ...
        images: formData.jobImages,
      };

      await apiClient.patch(`/jobs/${jobId}`, jobData);
