...
import InterviewCard from "@/components/job-seeker/InterviewCard";
...
export default function MyApplicationsPage() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      console.log("=== Next.js Applications Response ===");
    ...
    <div className="px-4 sm:px-0">
        ...
        return (
          <div className="mb-8">
            <motion.div
              ...
              className="bg-linear-to-r from-primary/10 via-secondary/10 to-primary/10 dark:from-primary/20 dark:via-secondary/20 dark:to-primary/20 border border-primary/30 dark:border-primary/40 rounded-xl p-6 mb-6"
            >
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {upcomingInterviews.slice(0, 3).map((interview) => (
                  <InterviewCard
                    ...
                    className="bg-card/80 backdrop-blur-sm shadow-lg border-primary/20"
                  />
                ))}
              </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-secondary text-secondary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-secondary/90 transition-colors"
            >
              <svg
                className="w-4 h-4"
                fill="none"
        ...
        {filteredApplications.length === 0 ? (
          ...
          filteredApplications.map((application, index) => (
            <motion.div
              ...
              className="bg-card border-2 border-border rounded-xl p-6 hover:shadow-lg transition-all duration-200 hover:border-primary/30"
            >
              <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium border-2 flex items-center gap-1 ${getStatusColor(
                        application.status
                      )}`}
                    >
                    ...
                    {(application.job.salaryMin ||
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {application.job.salaryMin && application.job.salaryMax
                          ? `GHS ${application.job.salaryMin.toLocaleString()} - GHS ${application.job.salaryMax.toLocaleString()}`
                          : application.job.salaryMin
                          ? `GHS ${application.job.salaryMin.toLocaleString()}+`
                          : `Up to GHS ${application.job.salaryMax?.toLocaleString()}`}
                      </div>
                    )}
                  </div>
                ...
                <div className="flex lg:flex-col gap-2">
                  <Link
                    ...
                    className="flex items-center justify-center gap-2 bg-muted hover:bg-muted/80 text-foreground px-4 py-2 rounded-lg text-sm transition-colors border border-border"
                  >
              ...
              <div className="mt-6 pt-4 border-t-2 border-border">
                ...
                <div className="w-full bg-muted/50 dark:bg-muted/30 rounded-full h-2 border border-border">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      application.status === "PENDING"
                        ? "w-1/4 bg-yellow-500 dark:bg-yellow-400"
                        : application.status === "REVIEWED"
                        ? "w-2/4 bg-blue-500 dark:bg-blue-400"
                        : application.status === "SHORTLISTED"
                        ? "w-3/4 bg-purple-500 dark:bg-purple-400"
                        : application.status === "HIRED"
                        ? "w-full bg-green-500 dark:bg-green-400"
                        : application.status === "REJECTED"
                        ? "w-full bg-red-500 dark:bg-red-400"
      ...
      {applications.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-8 bg-blue-50 dark:bg-blue-950/30 border-2 border-blue-200 dark:border-blue-800 rounded-xl p-6"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-blue-800 dark:text-blue-200">
            <div className="flex items-start gap-2">
              <MdPending className="w-4 h-4 text-yellow-600 dark:text-yellow-400 mt-0.5 shrink-0" />
              <div>
            ...
            <div className="flex items-start gap-2">
              <MdVisibility className="w-4 h-4 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
              <div>
            ...
            <div className="flex items-start gap-2">
              <MdStar className="w-4 h-4 text-purple-600 dark:text-purple-400 mt-0.5 shrink-0" />
              <div>
                <strong>Shortlisted:</strong> You&apos;ve been shortlisted for
                the next stage!
              </div>
            </div>
            <div className="flex items-start gap-2">
              <MdCheckCircle className="w-4 h-4 text-green-600 dark:text-green-400 mt-0.5 shrink-0" />
              <div>
                <strong>Hired:</strong> Congratulations! You&apos;ve been
                selected for the position.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <MdCancel className="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5 shrink-0" />
              <div>
                <strong>Rejected:</strong> Unfortunately, you weren&apos;t
                selected for this position.
              </div>
            </div>
            <div className="flex items-start gap-2">
              <MdSchedule className="w-4 h-4 text-primary mt-0.5 shrink-0" />
              <div>
